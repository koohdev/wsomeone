...
Passes a [CSP nonce⁠](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Global_attributes/nonce) through to NumberFlow’s inline `<style>` elements during SSR and hydration.

```
<NumberFlow value={123} nonce={myNonce} />
```

Hash-based CSP instructions NumberFlow exports its styles as strings so you can hash them for your `Content-Security-Policy`. Here’s an example:
...
Can be used to adjust the spacing between digits during spin animations. The examples on this site all use `0.85`.
