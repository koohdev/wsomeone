  ...
  return (
    <div className="min-h-screen w-full bg-[#EDEDEF] text-[#C10016] select-none font-sans flex flex-col justify-center items-center px-5 py-12">
      <div className="w-full max-w-sm flex flex-col items-center">
        {/* Brand Title */}
        <h1
          className="text-sm sm:text-base font-bold tracking-[0.25em] uppercase text-center mb-8 text-[#C10016]"
          style={{ textShadow: '0 0.4px 0.4px rgba(193, 0, 22, 0.15)' }}
        >
          WSOMEONE
        </h1>

        {/* Clean vertical stack of rounded cotton paper cards */}
        <div className="w-full flex flex-col gap-3">
          {decks.map((deck) => (
            <button
              key={deck.id}
              onClick={() => onSelectDeck(deck)}
              className="relative w-full rounded-2xl p-5 text-center border active:scale-[0.98] transition-all cursor-pointer group overflow-hidden"
              style={{
            ...
            >
              ...
              <h2
                className="relative z-10 text-[#C10016] text-sm sm:text-base font-bold uppercase tracking-tight"
                style={{ textShadow: '0 0.4px 0.4px rgba(193, 0, 22, 0.15)' }}
              >
                {deck.title}
              </h2>
